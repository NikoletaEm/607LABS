...
```
```{r}
## I exported the database from MySQL using Table Export Wizzard, exporting into a csv file, which I imported in Rstudio using the following commands.
```
```{r}
data<-read.csv('pop_movies.csv')
